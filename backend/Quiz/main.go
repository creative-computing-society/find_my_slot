...
func FindMyQuizSlot(c *gin.Context) {
	userID, err := login.GetUIDFromSession(c)
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
		return
	...
	err = db.User_Questions.Coll.FindOne(
		db.User_Questions.Context,
		bson.M{"userID": userID},
		options.FindOne().SetProjection(bson.M{"shift": 1, "_id": 0}),
	).Decode(&result)
	if err != nil {
	...
	slotMap := map[int]string{
		1: "9:10 PM to 9:25 PM",
		2: "9:35 PM to 9:50 PM",
		3: "10:00 PM to 10:15 PM",
		4: "10:30 PM to 10:45 PM",
		5: "11:00 PM to 11:15 PM",
	}

	slotTime, ok := slotMap[result.Shift]
	if !ok {
		c.JSON(http.StatusNotFound, gin.H{"error": "Slot not Found"})
		return
	}
...
func FindMyPISlot(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"slot": "PI Slots not Announced Yet"})
}

func Announcement(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"note": "Slots for Quiz Announced	|	The Link for the Quiz Portal will Be Shared on Instagram Story(@ccs_tiet)	|	If you miss your quiz, it will automatically be rescheduled for the same day or the next day. Please check the portal again 10 minutes after the quiz ends."})
}

func FinalResult(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"result": "Not Yet Announced"})
}
